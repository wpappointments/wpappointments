...
if ( ! function_exists( 'wpappointments_render_booking_flow_block' ) ) {
	/**
	 * Render the booking flow block markup.
	 *
	 * @param array  $attributes Block attributes.
	 * @param string $content    Rendered InnerBlocks content (styled buttons).
	 *
	 * @return string HTML output.
	 */
	function wpappointments_render_booking_flow_block( $attributes, $content ) {
		/**
		...
		$element = sprintf(
			"<div class='wpappstip-appointments-flow' style='min-height:400px' data-attributes='%s' data-buttons='%s'></div>",
		...
		return apply_filters( 'wpappointments_booking_flow_output', $block_output, $attributes );
	}
}

echo wp_kses_post( wpappointments_render_booking_flow_block( $attributes, $content ) );
