...
function formatTimeSummary(schedule: Schedule): string {
	...
	if (enabledDays.length === 0) {
		return __('No working days', 'appstip-appointments');
	}

	if (enabledDays.length === 7) {
		return __('Every day', 'appstip-appointments');
	}

	const dayLabels: Record<Day, string> = {
		monday: __('Mon', 'appstip-appointments'),
		tuesday: __('Tue', 'appstip-appointments'),
		wednesday: __('Wed', 'appstip-appointments'),
		thursday: __('Thu', 'appstip-appointments'),
		friday: __('Fri', 'appstip-appointments'),
		saturday: __('Sat', 'appstip-appointments'),
		sunday: __('Sun', 'appstip-appointments'),
	};
				...
				<div className={styles.scheduleHeader}>
					...
					{schedule.isDefault && (
						<span className={styles.badgeDefault}>
							{__('Default', 'appstip-appointments')}
						</span>
					)}
					<span
						className={
...
function ScheduleEditor({
		...
		() => [
			{
				id: 'name',
				type: 'text' as const,
				label: __('Schedule name', 'appstip-appointments'),
				isValid: { required: true },
				Edit: TextInput,
			},
			{
				id: 'timezone',
				label: __('Timezone', 'appstip-appointments'),
	...
	const openDangerZone = () => {
		openSlideOut({
			id: dangerSlideoutId,
			parentId: slideoutId,
			title: __('Danger zone', 'appstip-appointments'),
			content: (
				<DangerZoneContent
	...
	return (
		<>
			{canDelete && isTopSlideout && (
				<SlideoutHeaderActionsFill>
					<Tooltip
						text={__('Delete schedule', 'appstip-appointments')}
					>
						<WPButton
							icon={trash}
							isDestructive
			...
			<div className={styles.editorContent}>
				...
				<div>
					...
					>
						{__('Working hours', 'appstip-appointments')}
					</Text>
					{orderedDays.map((day) => (
				...
				{!isNew && (
					<ToggleControl
						__nextHasNoMarginBottom
						label={__('Default schedule', 'appstip-appointments')}
				...
				>
					{isNew
						? __('Create schedule', 'appstip-appointments')
...
function DangerZoneContent({
	...
	return (
		<div className={styles.dangerZoneContent}>
			...
			>
				{__('Delete schedule', 'appstip-appointments')}
			</Button>

			{showConfirm && (
				<DeleteModal
					title={__('Delete Schedule', 'appstip-appointments')}
					message={
						reassign
							? __(
...
export default function ScheduleSettings() {
	...
	const handleCreate = () => {
		...
		openSlideOut({
			id: 'schedule-new',
			title: __('New schedule', 'appstip-appointments'),
	...
	return (
		<Card className={globalStyles.card}>
			<CardHeader>
				<Text size="title">
					{__('Schedules', 'appstip-appointments')}
				</Text>
			</CardHeader>
			...
			<CardFooter>
				<Button variant="secondary" onClick={handleCreate}>
					{__('Add schedule', 'appstip-appointments')}
				</Button>
			</CardFooter>
		</Card>
	);
