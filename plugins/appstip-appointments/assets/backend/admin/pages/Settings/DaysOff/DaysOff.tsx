...
const REASON_LABELS: Record<OooReason, string> = {
	unspecified: __('Unspecified', 'appstip-appointments'),
...
function OooEditor({ entry }: { entry?: OooEntry }) {
	...
	const fields: Field<OooFormData>[] = [
		{
			id: 'reason',
			type: 'text',
			label: __('Reason', 'appstip-appointments'),
		...
		},
		{
			id: 'notes',
			type: 'text',
			label: __('Notes', 'appstip-appointments'),
			Edit: TextareaInput,
		},
		{
			id: 'note_public',
	...
	const handleSubmit = async () => {
		if (!rangeStart) {
			setError(__('Please select a date range', 'appstip-appointments'));
			return;
		}
	...
	return (
		<>
			{entry && isTopSlideout && (
				<SlideoutHeaderActionsFill>
					<Tooltip
						text={__('Delete time off', 'appstip-appointments')}
					>
						<Button
							icon={trash}
							isDestructive
			...
			<div className={styles.editorContent}>
				<div>
					<p className={styles.dateLabel}>
						{rangeStart && rangeEnd
							...
							: rangeStart
								? `${formatYmd(rangeStart)} — ${__('select end date', 'appstip-appointments')}`
								: __(
										'Select date range',
										'appstip-appointments'
									)}
				...
				>
					{entry
						...
						: __('Add time off', 'appstip-appointments')}
				</Button>
			</div>
		</>
	);
...
export default function DaysOffSettings() {
	...
	const handleCreate = () => {
		openSlideOut({
			id: 'ooo-new',
			title: __('Add time off', 'appstip-appointments'),
			content: <OooEditor />,
		});
	};
	...
	return (
		<>
			<Card className={globalStyles.card}>
				<CardHeader>
					<Text size="title">
						{__('Your time off', 'appstip-appointments')}
					</Text>
				</CardHeader>
