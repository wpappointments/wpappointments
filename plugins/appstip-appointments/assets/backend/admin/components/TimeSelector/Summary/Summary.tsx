		...
		<>
			<Card className={styles.summary}>
				<CardHeader className={styles.summaryHeader}>
					{__('Date and time summary', 'appstip-appointments')}:{' '}
					{headerActions}
				</CardHeader>
				<CardBody className={styles.summaryBody}>
					<div className={styles.summaryRow}>
						<strong>{__('Date', 'appstip-appointments')}</strong>{' '}
						{date && formatDate(date)}
					</div>
					<div className={styles.summaryRow}>
						<strong>{__('Time', 'appstip-appointments')}</strong>{' '}
						{/* translators: %s: start time, %s: end time */}
						{sprintf(
							__('%s to %s', 'appstip-appointments'),
					...
					<div className={styles.summaryRow}>
						<strong>
							{__('Duration', 'appstip-appointments')}
						</strong>{' '}
						{/* TODO: use _n translation for this */}
						{duration} {__('minutes', 'appstip-appointments')}
					</div>
				</CardBody>
			</Card>
			{showAvailabilityWarning && (
				<div
					className={`${styles.summaryRow} ${styles.summaryRowError}`}
				>
					{__(
