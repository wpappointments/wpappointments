...
export default function AppointmentsTableFull() {
	...
	if (!appointments || appointments.length === 0) {
		return (
			<TableFullEmpty>
				<p>
					{__('You have no appointments yet', 'appstip-appointments')}
				</p>
				<Button variant="primary" onClick={addAppointment}>
					{__('New Appointment', 'appstip-appointments')}
				</Button>
			</TableFullEmpty>
		);
	}
	...
	const fields: Field<Appointment>[] = [
		{
			id: 'title',
			label: __('Title', 'appstip-appointments'),
			render: ({ item }) => {
				return (
					<>
						<Button
		...
		{
			id: 'date',
			label: __('Date', 'appstip-appointments'),
			render: ({ item }) => {
		...
		{
			id: 'time',
			label: __('Time', 'appstip-appointments'),
			render: ({ item }) => {
	...
	const actions: Action<Appointment>[] = [
		{
			id: 'view',
			icon: info,
			label: __('View appointment details', 'appstip-appointments'),
			callback: (items) => {
		...
		},
		{
			id: 'edit',
			icon: edit,
			label: __('Edit appointment details', 'appstip-appointments'),
			callback: (items) => {
		...
		},
		{
			...
			label: __('Cancel appointment', 'appstip-appointments'),
			callback: (items) => {
		...
		},
		{
			...
			label: __('Confirm appointment', 'appstip-appointments'),
			callback: (items) => {
		...
		},
		{
			...
			label: __('Delete appointment', 'appstip-appointments'),
			callback: (items) => {
				const { id, status } = items[0];
				setAppointmentModal({ id, status });
			},
