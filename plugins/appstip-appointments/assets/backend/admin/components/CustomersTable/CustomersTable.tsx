...
export default function CustomersTable() {
	...
	if (!customers || totalItems === 0) {
		return (
			<TableFullEmpty>
				<p>{__('You have no customers yet', 'appstip-appointments')}</p>
				<Button variant="primary" onClick={addCustomer}>
					{__('Create New Customer', 'appstip-appointments')}
				</Button>
			</TableFullEmpty>
		);
	}
	...
	const fields: Field<Customer>[] = [
		{
			id: 'name',
			label: __('Name', 'appstip-appointments'),
			render: ({ item }) => {
				return (
		...
		{
			id: 'phone',
			label: __('Phone', 'appstip-appointments'),
			render: ({ item }) => {
		...
		{
			id: 'created',
			label: __('Created', 'appstip-appointments'),
			render: ({ item }) => {
				return (
					<>
						{item.created
	...
	const actions: Action<Customer>[] = [
		{
			id: 'view',
			icon: info,
			label: __('View customer details', 'appstip-appointments'),
			callback: (items) => {
		...
		},
		{
			id: 'edit',
			icon: edit,
			label: __('Edit customer details', 'appstip-appointments'),
			callback: (items) => {
		...
		},
		{
			id: 'delete',
			icon: trash,
			label: __('Delete customer', 'appstip-appointments'),
			RenderModal: ({ items, closeModal: closeActionModal }) => {
				const item = items[0];
				return (
					<DeleteModal
						title={__('Delete Customer', 'appstip-appointments')}
						message={__(
