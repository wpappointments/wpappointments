...
export default function CustomerSelector({
	...
	return (
		<SlideOut
			title={__('Select Customer', 'appstip-appointments')}
			id="select-customer"
		>
			<InputControl
				label={__('Search for a customer', 'appstip-appointments')}
				placeholder={__(
			...
			{filteredCustomers && filteredCustomers.length > 0 && (
				<>
					<div className={styles.tableHeader}>
						<span>{__('Name', 'appstip-appointments')}</span>
						<span>{__('Email', 'appstip-appointments')}</span>
						<span>{__('Phone', 'appstip-appointments')}</span>
					</div>
