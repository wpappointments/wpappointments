		...
		<>
			{!hideProgressBar && (
				<header className={multiStepStyles.stepsHeader}>
					<button
						...
						data-step={1}
					>
						{__('Select date', 'appstip-appointments')}
					</button>
					<button
						type="button"
						className={cn({
						...
						data-step={2}
					>
						{__('About you', 'appstip-appointments')}
					</button>
					<div
						className={cn({
							[multiStepStyles.stepsHeaderLabel]: true,
						...
						data-step={3}
					>
						{__('Summary', 'appstip-appointments')}
					</div>
				</header>
			)}
				...
				<div>
					...
					{!hideStepTitles && (
						<h2 ref={stepHeadingRef} tabIndex={-1}>
							{__('Select date and time', 'appstip-appointments')}
						</h2>
					)}
					<BookingFlowCalendar
						onSlotSelected={
			...
			{activeStep === 1 && (
				...
				>
					...
					{!hideStepTitles && (
						<h2 ref={stepHeadingRef} tabIndex={-1}>
							{__('Customer information', 'appstip-appointments')}
						</h2>
					)}
					<BookingFlowCustomer />
