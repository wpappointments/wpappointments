...
export default function CalendarDay({
	...
	if (hasNotices) {
		availabilityLabel = sprintf(
			...
			__('has notice: %s', 'appstip-appointments'),
			noticeTitle ?? __('unavailable', 'appstip-appointments')
		);
	} else if (!isInFutureRange) {
		availabilityLabel = __('past date', 'appstip-appointments');
	} else if (!day.available || totalAvailable === 0) {
		availabilityLabel = __('no slots available', 'appstip-appointments');
	} else {
		availabilityLabel = sprintf(
