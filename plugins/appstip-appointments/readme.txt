...
Appstip Appointments is a modern appointment scheduling plugin for WordPress. It ships a full-featured booking system with bookable entities, service variants, availability management, and an admin interface built with React.
...
1. Upload the `appstip-appointments` folder to the `/wp-content/plugins/` directory, or install the plugin through the WordPress Plugins screen.
...
Yes. All user-facing strings use the `appstip-appointments` text domain and ready-made translations are bundled for 10 locales. Contributions are welcome via WordPress.org's translation platform.
...
= 0.2.0 =
* Renamed plugin slug to `appstip-appointments` for WordPress.org compatibility.
...
= 0.2.0 =
This release renames the plugin slug to `appstip-appointments` to comply with WordPress.org trademark rules. Existing install paths, menu URLs, and the plugin text domain have changed. After updating, re-activate the plugin if the admin menu does not appear.
