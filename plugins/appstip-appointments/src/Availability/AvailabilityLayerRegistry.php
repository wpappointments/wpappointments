	...
	public function register( $slug, $priority, $config ) {
		if ( isset( $this->layers[ $slug ] ) ) {
			return new WP_Error(
				...
				sprintf( __( 'Availability layer "%s" is already registered', 'appstip-appointments' ), $slug )
			);
		}
		...
		if ( ! in_array( $type, array( 'base', 'narrowing' ), true ) ) {
			return new WP_Error(
				...
				__( 'Availability layer type must be "base" or "narrowing"', 'appstip-appointments' )
			);
		}
		...
		if ( ! isset( $config['callback'] ) || ! is_callable( $config['callback'] ) ) {
			return new WP_Error(
				...
				__( 'Availability layer must have a callable callback', 'appstip-appointments' )
			);
		}
