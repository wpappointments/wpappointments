...
class Customer {
	...
	public function save() {
		if ( ! is_array( $this->user ) ) {
			return new WP_Error(
				...
				__( 'When saving user, you have to provide user data array in constructor', 'appstip-appointments' )
			);
		}
	...
	public function update( $update ) {
		...
		if ( is_array( $this->user ) ) {
			return new WP_Error(
				...
				__( 'When updating customer you have to initialize model with user object or user id', 'appstip-appointments' )
			);
		}
	...
	public function delete() {
		...
		if ( ! $deleted ) {
			return new \WP_Error(
				...
				__( 'Could not delete customer', 'appstip-appointments' )
			);
		}
	...
	protected function validate_user_id( $user_id ) {
		if ( ! $user_id ) {
			return new \WP_Error( 'user_id_required', __( 'User ID is required', 'appstip-appointments' ) );
		}

		$user = get_user_by( 'ID', $user_id );

		if ( ! $user ) {
			return new \WP_Error( 'user_not_found', __( 'User not found', 'appstip-appointments' ) );
		}

		if ( ! $user->has_cap( 'wpa-customer' ) ) {
			return new \WP_Error( 'user_not_customer', __( 'User is not a customer', 'appstip-appointments' ) );
		}
