...
class OutOfOffice {
		...
		if ( $ooo instanceof WP_Post ) {
			if ( PluginInfo::POST_TYPES['ooo'] !== $ooo->post_type ) {
				$this->ooo = new WP_Error(
					...
					__( 'Post is not an OOO entry', 'appstip-appointments' )
				);
				return;
			}
			$this->ooo = $ooo;
			...
			$this->ooo = new WP_Error(
				'ooo_invalid_type',
				__( 'Invalid value passed to OutOfOffice constructor', 'appstip-appointments' )
			);
		}
	}
	...
	public function save() {
		...
		if ( $this->ooo instanceof WP_Post ) {
			return new WP_Error(
				...
				__( 'This OOO entry is already persisted. Use update() instead', 'appstip-appointments' )
			);
		}
	...
	public function update( $data ) {
		...
		if ( ! $this->ooo ) {
			return new WP_Error(
				'ooo_object_expected',
				__( 'OOO entry not found', 'appstip-appointments' )
			);
		}
	...
	public function delete() {
		...
		if ( ! $deleted ) {
			return new WP_Error(
				'ooo_delete_failed',
				__( 'Failed to delete OOO entry', 'appstip-appointments' )
			);
		}
	...
	private static function generate_title( $reason, $start_date, $end_date ) {
		$labels = array(
			'unspecified' => __( 'Time off', 'appstip-appointments' ),
			'vacation'    => __( 'Vacation', 'appstip-appointments' ),
			'travel'      => __( 'Travel', 'appstip-appointments' ),
			'sick_leave'  => __( 'Sick Leave', 'appstip-appointments' ),
			'holiday'     => __( 'Holiday', 'appstip-appointments' ),
		);
	...
	private function parse_data( $data ) {
		if ( empty( $data['start_date'] ) || empty( $data['end_date'] ) ) {
			$this->ooo = new WP_Error(
				'ooo_dates_required',
				__( 'Start date and end date are required', 'appstip-appointments' )
			);
			return;
		}
	...
	private function parse_from_id( $id ) {
		...
		if ( ! $post ) {
			$this->ooo = new WP_Error( 'ooo_not_found', __( 'OOO entry not found', 'appstip-appointments' ) );
			return;
		}

		if ( PluginInfo::POST_TYPES['ooo'] !== $post->post_type ) {
			$this->ooo = new WP_Error( 'ooo_invalid_type', __( 'Post is not an OOO entry', 'appstip-appointments' ) );
			return;
		}
