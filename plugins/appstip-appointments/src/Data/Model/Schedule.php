...
class Schedule {
		...
		if ( $schedule instanceof WP_Post ) {
			if ( PluginInfo::POST_TYPES['schedule'] !== $schedule->post_type ) {
				$this->schedule = new WP_Error(
					...
					__( 'Post is not a schedule. Expected post_type wpa-schedule', 'appstip-appointments' )
				);
				return;
			}
			...
			$this->schedule = new WP_Error(
				...
				__( 'Schedule value passed to constructor cannot be null', 'appstip-appointments' )
			);
		} else {
			$this->schedule = new WP_Error(
				'schedule_invalid_type',
				__( 'Schedule value passed to constructor is invalid. Expected array, int, string or WP_Post', 'appstip-appointments' )
			);
		}
	}
	...
	public function save() {
		...
		if ( $this->schedule instanceof WP_Post ) {
			return new WP_Error(
				...
				__( 'This schedule is already persisted. Use update() instead', 'appstip-appointments' )
			);
		}
	...
	public function update( $data ) {
		...
		if ( ! $this->schedule ) {
			return new WP_Error(
				...
				__( 'Schedule not found. Instantiate Schedule class with a schedule object', 'appstip-appointments' )
			);
		}
		...
		if ( isset( $data['name'] ) ) {
			if ( '' === trim( $data['name'] ) ) {
				return new WP_Error( 'schedule_name_required', __( 'Schedule name is required', 'appstip-appointments' ), array( 'status' => 422 ) );
			}
	...
	public function delete( $reassign = true ) {
		...
		if ( ! $this->schedule ) {
			return new WP_Error(
				'schedule_object_expected',
				__( 'Schedule not found', 'appstip-appointments' )
			);
		}
		...
		if ( absint( $default_id ) === $id ) {
			return new WP_Error(
				...
				__( 'Cannot delete the default schedule', 'appstip-appointments' ),
				array( 'status' => 422 )
			);
		}
		...
		if ( ! $deleted ) {
			return new WP_Error(
				...
				__( 'Failed to delete schedule', 'appstip-appointments' )
			);
		}
	...
	private function validate_post_id( $post_id ) {
		if ( ! $post_id ) {
			return new WP_Error( 'schedule_id_required', __( 'Schedule ID is required', 'appstip-appointments' ) );
		}

		$post = get_post( $post_id );

		if ( ! $post ) {
			return new WP_Error( 'schedule_not_found', __( 'Schedule not found', 'appstip-appointments' ) );
		}

		if ( PluginInfo::POST_TYPES['schedule'] !== $post->post_type ) {
			return new WP_Error( 'schedule_invalid_type', __( 'Post is not a schedule', 'appstip-appointments' ) );
		}
	...
	private function validate_schedule_data( $data ) {
		if ( ! isset( $data['name'] ) || '' === trim( $data['name'] ) ) {
			return new WP_Error( 'schedule_name_required', __( 'Schedule name is required', 'appstip-appointments' ) );
		}
