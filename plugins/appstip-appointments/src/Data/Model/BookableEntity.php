...
class BookableEntity {
		...
		if ( $bookable instanceof WP_Post ) {
			if ( PluginInfo::POST_TYPES['bookable'] !== $bookable->post_type ) {
				$this->bookable = new WP_Error(
					...
					__( 'Post is not a bookable entity. Expected post_type wpa-bookable', 'appstip-appointments' )
				);
				return;
			}
			...
			$this->bookable = new WP_Error(
				...
				__( 'Bookable value passed to constructor cannot be null. Expected array, int, string or WP_Post', 'appstip-appointments' )
			);
		} else {
			$this->bookable = new WP_Error(
				'bookable_invalid_type',
				__( 'Bookable value passed to constructor is invalid. Expected array, int, string or WP_Post', 'appstip-appointments' )
			);
		}
	}
	...
	public function save() {
		...
		if ( $this->bookable instanceof WP_Post ) {
			return new WP_Error(
				...
				__( 'This bookable entity is already persisted. Use update() instead', 'appstip-appointments' )
			);
		}
	...
	public function update( $data ) {
		...
		if ( ! $this->bookable ) {
			return new WP_Error(
				...
				__( 'Bookable not found. Instantiate BookableEntity class with a bookable object', 'appstip-appointments' )
			);
		}
		...
		if ( 0 === $result ) {
			return new WP_Error(
				...
				__( 'Failed to update bookable entity', 'appstip-appointments' )
			);
		}
	...
	public function delete() {
		...
		if ( ! $deleted ) {
			return new WP_Error(
				...
				__( 'Failed to delete bookable entity', 'appstip-appointments' )
			);
		}
	...
	private function validate_post_id( $post_id ) {
		if ( ! $post_id ) {
			return new WP_Error( 'bookable_id_required', __( 'Bookable ID is required', 'appstip-appointments' ) );
		}

		$post = get_post( $post_id );

		if ( ! $post ) {
			return new WP_Error( 'bookable_not_found', __( 'Bookable not found', 'appstip-appointments' ) );
		}

		if ( PluginInfo::POST_TYPES['bookable'] !== $post->post_type ) {
			return new WP_Error( 'bookable_invalid_type', __( 'Post is not a bookable entity', 'appstip-appointments' ) );
		}
	...
	private function validate_bookable_data( $data ) {
		if ( ! isset( $data['name'] ) || '' === trim( $data['name'] ) ) {
			return new WP_Error( 'bookable_name_required', __( 'Bookable name is required', 'appstip-appointments' ) );
		}
