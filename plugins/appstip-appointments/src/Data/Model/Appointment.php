...
class Appointment {
	...
	public function cancel() {
		...
		if ( ! $cancelled ) {
			return new \WP_Error(
				...
				__( 'Appointment is already cancelled', 'appstip-appointments' )
			);
		}
	...
	public function confirm() {
		...
		if ( ! $confirmed ) {
			return new \WP_Error(
				...
				__( 'Appointment is already confirmed', 'appstip-appointments' )
			);
		}
	...
	public function delete() {
		...
		if ( ! $status ) {
			return new \WP_Error(
				...
				__( 'Appointment not found', 'appstip-appointments' )
			);
		}
		...
		if ( 'cancelled' !== $status ) {
			return new \WP_Error(
				...
				__( 'Appointment must be cancelled before deleting', 'appstip-appointments' )
			);
		}
	...
	protected function validate_post_id( $post_id ) {
		if ( ! $post_id ) {
			return new \WP_Error( 'appointment_id_required', __( 'Appointment ID is required', 'appstip-appointments' ) );
		}

		$post = get_post( $post_id );

		if ( ! $post ) {
			return new \WP_Error( 'appointment_not_found', __( 'Appointment not found', 'appstip-appointments' ) );
		}

		if ( 'wpa-appointment' !== $post->post_type ) {
			return new \WP_Error( 'appointment_invalid_type', __( 'Post is not an appointment', 'appstip-appointments' ) );
		}
