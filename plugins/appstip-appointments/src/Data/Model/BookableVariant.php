...
class BookableVariant {
		...
		if ( $variant instanceof WP_Post ) {
			if ( PluginInfo::POST_TYPES['bookable-variant'] !== $variant->post_type ) {
				$this->variant = new WP_Error(
					...
					__( 'Post is not a bookable variant. Expected post_type wpa-bookable-variant', 'appstip-appointments' )
				);
				return;
			}
			...
			$this->variant = new WP_Error(
				...
				__( 'Variant value passed to constructor cannot be null. Expected array, int, string or WP_Post', 'appstip-appointments' )
			);
		} else {
			$this->variant = new WP_Error(
				'variant_invalid_type',
				__( 'Variant value passed to constructor is invalid. Expected array, int, string or WP_Post', 'appstip-appointments' )
			);
		}
	}
	...
	public function save() {
		...
		if ( ! $parent_id ) {
			return new WP_Error(
				...
				__( 'Variant must have a parent bookable entity', 'appstip-appointments' )
			);
		}
		...
		if ( ! $parent || PluginInfo::POST_TYPES['bookable'] !== $parent->post_type ) {
			return new WP_Error(
				...
				__( 'Parent must be a valid bookable entity', 'appstip-appointments' )
			);
		}
	...
	public function update( $data ) {
		...
		if ( ! $this->variant ) {
			return new WP_Error(
				...
				__( 'Variant not found. Instantiate BookableVariant class with a variant object', 'appstip-appointments' )
			);
		}
		...
		if ( 0 === $result ) {
			return new WP_Error(
				...
				__( 'Failed to update bookable variant', 'appstip-appointments' )
			);
		}
	...
	public function delete() {
		...
		if ( ! $deleted ) {
			return new WP_Error(
				...
				__( 'Failed to delete bookable variant', 'appstip-appointments' )
			);
		}
	...
	public static function generate_from_matrix( $entity_id ) {
		...
		if ( ! $entity || PluginInfo::POST_TYPES['bookable'] !== $entity->post_type ) {
			return new WP_Error(
				...
				__( 'Invalid bookable entity for variant generation', 'appstip-appointments' )
			);
		}
	...
	private function validate_post_id( $post_id ) {
		if ( ! $post_id ) {
			return new WP_Error( 'variant_id_required', __( 'Variant ID is required', 'appstip-appointments' ) );
		}

		$post = get_post( $post_id );

		if ( ! $post ) {
			return new WP_Error( 'variant_not_found', __( 'Variant not found', 'appstip-appointments' ) );
		}

		if ( PluginInfo::POST_TYPES['bookable-variant'] !== $post->post_type ) {
			return new WP_Error( 'variant_invalid_type', __( 'Post is not a bookable variant', 'appstip-appointments' ) );
		}
