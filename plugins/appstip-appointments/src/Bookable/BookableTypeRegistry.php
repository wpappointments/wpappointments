	...
	public function register( $slug, $handler_class ) {
		if ( isset( $this->types[ $slug ] ) ) {
			return new WP_Error(
				...
				sprintf( __( 'Bookable type "%s" is already registered', 'appstip-appointments' ), $slug )
			);
		}
		...
		if ( ! class_exists( $handler_class ) ) {
			return new WP_Error(
				...
				sprintf( __( 'Bookable type handler class "%s" not found', 'appstip-appointments' ), $handler_class )
			);
		}
		...
		if ( ! is_subclass_of( $handler_class, AbstractBookableTypeHandler::class ) ) {
			return new WP_Error(
				...
				/* translators: %s: handler class name */
				sprintf( __( 'Bookable type handler "%s" must extend AbstractBookableTypeHandler', 'appstip-appointments' ), $handler_class )
			);
		}
