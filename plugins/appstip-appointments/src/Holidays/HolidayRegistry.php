...
class HolidayRegistry {
	...
	public static function add_group( array $data ) {
		...
		if ( ! in_array( $type, self::ALLOWED_TYPES, true ) ) {
			return new \WP_Error(
				'invalid_type',
				__( 'Invalid holiday group type.', 'appstip-appointments' ),
				array( 'status' => 422 )
			);
		}
		...
		if ( null === $source ) {
			return new \WP_Error(
				'file_not_found',
				__( 'Holiday data file not found.', 'appstip-appointments' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( $source_type !== $type ) {
			return new \WP_Error(
				'type_mismatch',
				__( 'Source file type does not match requested type.', 'appstip-appointments' ),
				array( 'status' => 422 )
			);
		}
		...
		foreach ( $groups as $group ) {
			if ( ( $group['id'] ?? '' ) === $group_id ) {
				return new \WP_Error(
					'duplicate_group',
					__( 'This holiday group is already added.', 'appstip-appointments' ),
					array( 'status' => 422 )
				);
			}
		}
