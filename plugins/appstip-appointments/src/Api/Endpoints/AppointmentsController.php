...
use WPAppointments\Api\PublicEndpointGuard;
...
class AppointmentsController extends Controller {
			...
			array(
				array(
					...
					'permission_callback' => function () {
						return current_user_can( Capabilities::VIEW_APPOINTMENTS );
					},
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => function () {
						return current_user_can( Capabilities::CREATE_APPOINTMENTS );
					},
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => function () {
						return current_user_can( Capabilities::EDIT_APPOINTMENTS );
					},
				),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => function () {
						return current_user_can( Capabilities::DELETE_APPOINTMENTS );
					},
				),
			)
		);
	...
	public static function init_public_routes() {
		// Public booking submission. Visitors are unauthenticated by
		// design; PublicEndpointGuard layers a honeypot trap and a
		// per-IP rate limit (5 requests / 15 min) so the endpoint
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => array( PublicEndpointGuard::class, 'booking_create_check' ),
				),
			)
		);
	}
	...
	public static function get_all_appointments( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Appointments fetched successfully', 'appstip-appointments' ),
	...
	public static function get_upcoming_appointments( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Upcoming appointments fetched successfully', 'appstip-appointments' ),
			self::paginated( 'appointments', $results )
		);
	}
	...
	public static function create_appointment( WP_REST_Request $request ) {
		...
		if ( ! in_array( $status, $allowed_statuses, true ) ) {
			return self::error(
				new \WP_Error( 'invalid_status', __( 'Invalid appointment status', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}

		if ( $duration < 1 ) {
			return self::error(
				new \WP_Error( 'invalid_duration', __( 'Duration must be greater than zero', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}
		...
		if ( false === $date ) {
			return self::error(
				new \WP_Error( 'invalid_date', __( 'Invalid date format', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}
		...
		return self::response(
			__( 'Appointment created successfully', 'appstip-appointments' ),
			array(
	...
	public static function create_appointment_public( WP_REST_Request $request ) {
		...
		$appointment       = new Appointment(
			array(
				'title'          => $core_entity_name ? $core_entity_name : __( 'Appointment', 'appstip-appointments' ),
				'customer'       => $customer,
	...
	public static function update_appointment( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Appointment updated successfully', 'appstip-appointments' ),
			array(
	...
	public static function cancel_appointment( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Appointment cancelled successfully', 'appstip-appointments' ),
			array(
	...
	public static function delete_appointment( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Appointment deleted successfully', 'appstip-appointments' ),
			array(
	...
	public static function confirm_appointment( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Appointment confirmed successfully', 'appstip-appointments' ),
			array(
