...
class BookableAvailabilityController extends Controller {
	...
	public static function init() {
		// Three public availability reads. Booking-flow visitors are
		// unauthenticated by design, so `permission_callback` cannot
		// require a capability; the per-IP rate limit in
		// PublicEndpointGuard caps abusive scraping while leaving
		// legitimate browsing untouched.
		$availability_check = array( PublicEndpointGuard::class, 'availability_read_check' );
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => $availability_check,
				),
			)
		);
	...
	public static function get_variant_availability( $request ) {
		...
		return self::response(
			__( 'Availability fetched successfully', 'appstip-appointments' ),
			array(
	...
	public static function get_entity_availability( $request ) {
		...
		return self::response(
			__( 'Entity availability fetched successfully', 'appstip-appointments' ),
	...
	public static function get_calendar_slots( $request ) {
		...
		if ( ! in_array( $timezone, \DateTimeZone::listIdentifiers(), true ) ) {
			return self::error(
				new WP_Error( 'invalid_timezone', __( 'Invalid timezone', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}

		if ( ! is_string( $calendar_raw ) ) {
			return self::error(
				new WP_Error( 'missing_calendar', __( 'Calendar data is required', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}
		...
		if ( ! is_array( $calendar ) ) {
			return self::error(
				new WP_Error( 'invalid_calendar', __( 'Invalid calendar data', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}
		...
		if ( empty( $all_dates ) ) {
			return self::response(
				__( 'Calendar slots fetched successfully', 'appstip-appointments' ),
