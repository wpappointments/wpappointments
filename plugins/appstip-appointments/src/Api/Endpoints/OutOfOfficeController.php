...
class OutOfOfficeController extends Controller {
	...
	public static function get_all( $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- Required by REST callback contract.
		...
		return self::response(
			__( 'OOO entries fetched successfully', 'appstip-appointments' ),
	...
	public static function create( $request ) {
		...
		if ( empty( $data['start_date'] ) || empty( $data['end_date'] ) ) {
			return self::error(
				new WP_Error( 'ooo_dates_required', __( 'Start date and end date are required', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}

		if ( $data['start_date'] > $data['end_date'] ) {
			return self::error(
				new WP_Error( 'ooo_dates_inverted', __( 'Start date must be before end date', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}
		...
		return self::response(
			__( 'OOO entry created successfully', 'appstip-appointments' ),
	...
	public static function get_one( $request ) {
		...
		return self::response(
			__( 'OOO entry fetched successfully', 'appstip-appointments' ),
	...
	public static function update( $request ) {
		...
		return self::response(
			__( 'OOO entry updated successfully', 'appstip-appointments' ),
	...
	public static function delete_entry( $request ) {
		...
		return self::response(
			__( 'OOO entry deleted successfully', 'appstip-appointments' ),
			array( 'id' => $deleted )
		);
	}
	...
	public static function get_dates( $request ) {
		...
		if ( empty( $start_date ) || empty( $end_date ) ) {
			return self::error(
				new WP_Error( 'ooo_dates_required', __( 'start_date and end_date are required', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}
		...
		if ( ! $start_obj || $start_obj->format( 'Y-m-d' ) !== $start_date
			...
			return self::error(
				new WP_Error( 'ooo_dates_invalid', __( 'start_date and end_date must be valid Y-m-d dates', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}

		if ( $start_date > $end_date ) {
			return self::error(
				new WP_Error( 'ooo_dates_order', __( 'start_date must not be after end_date', 'appstip-appointments' ), array( 'status' => 422 ) )
			);
		}
		...
		return self::response(
			__( 'Blocked dates fetched successfully', 'appstip-appointments' ),
			array( 'dates' => $blocked )
		);
	}
	...
	private static function sanitize_ooo_data( $data ) {
		...
		if ( isset( $data['start_date'] ) ) {
			...
			if ( ! $obj || $obj->format( 'Y-m-d' ) !== $date ) {
				return new WP_Error( 'ooo_invalid_start_date', __( 'start_date must be a valid Y-m-d date', 'appstip-appointments' ), array( 'status' => 422 ) );
			}
		...
		if ( isset( $data['end_date'] ) ) {
			...
			if ( ! $obj || $obj->format( 'Y-m-d' ) !== $date ) {
				return new WP_Error( 'ooo_invalid_end_date', __( 'end_date must be a valid Y-m-d date', 'appstip-appointments' ), array( 'status' => 422 ) );
			}
		...
		if ( isset( $data['reason'] ) ) {
			...
			if ( ! in_array( $reason, OutOfOffice::ALLOWED_REASONS, true ) ) {
				return new WP_Error( 'ooo_invalid_reason', __( 'reason must be one of the allowed values', 'appstip-appointments' ), array( 'status' => 422 ) );
			}
