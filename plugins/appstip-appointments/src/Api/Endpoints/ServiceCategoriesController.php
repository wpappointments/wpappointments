...
class ServiceCategoriesController extends Controller {
	...
	public static function get_all_categories( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Service categories fetched successfully', 'appstip-appointments' ),
			array(
	...
	public static function create_category( WP_REST_Request $request ) {
		...
		if ( empty( $name ) ) {
			return self::error(
				new \WP_Error(
					...
					__( 'Category name is required', 'appstip-appointments' ),
					array( 'status' => 422 )
				)
			);
		}
		...
		return self::response(
			__( 'Service category created successfully', 'appstip-appointments' ),
			array(
				'category' => array(
					'id'    => $term->term_id,
					'name'  => $term->name,
	...
	public static function update_category( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Service category updated successfully', 'appstip-appointments' ),
			array(
				'category' => array(
					'id'    => $term->term_id,
					'name'  => $term->name,
	...
	public static function delete_category( WP_REST_Request $request ) {
		...
		return self::response(
			__( 'Service category deleted successfully', 'appstip-appointments' ),
			array(
				'categoryId' => $id,
			)
		);
