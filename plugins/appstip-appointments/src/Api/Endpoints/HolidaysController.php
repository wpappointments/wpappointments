...
class HolidaysController extends Controller {
	...
	public static function get_available( $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- Required by REST callback contract.
		return self::response(
			__( 'Available holiday sets fetched successfully', 'appstip-appointments' ),
			array(
	...
	public static function get_groups( $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- Required by REST callback contract.
		return self::response(
			__( 'Holiday groups fetched successfully', 'appstip-appointments' ),
	...
	public static function add_group( $request ) {
		...
		return self::response(
			__( 'Holiday group added successfully', 'appstip-appointments' ),
			array( 'groups' => HolidayRegistry::get_groups_with_holidays() )
		);
	}
	...
	public static function remove_group( $request ) {
		...
		return self::response(
			__( 'Holiday group removed successfully', 'appstip-appointments' ),
			array( 'groups' => HolidayRegistry::get_groups_with_holidays() )
		);
	}
	...
	public static function update_group( $request ) {
		...
		if ( null !== ( $params['enabled'] ?? null ) ) {
			...
			return self::response(
				__( 'Holiday group updated successfully', 'appstip-appointments' ),
				array( 'groups' => HolidayRegistry::get_groups_with_holidays() )
			);
		}
		...
		if ( isset( $params['holiday_key'] ) && null !== ( $params['holiday_enabled'] ?? null ) ) {
			...
			return self::response(
				__( 'Holiday updated successfully', 'appstip-appointments' ),
				array( 'groups' => HolidayRegistry::get_groups_with_holidays() )
			);
		}
		...
		return self::error(
			new WP_Error(
				'no_update_data',
				__( 'No update data provided.', 'appstip-appointments' ),
				array( 'status' => 422 )
			)
		);
	}
	...
	public static function get_group_holidays( $request ) {
		...
		if ( null === $group ) {
			return self::error(
				new WP_Error(
					'group_not_found',
					__( 'Holiday group not found.', 'appstip-appointments' ),
					array( 'status' => 404 )
				)
			);
		}
		...
		return self::response(
			__( 'Group holidays fetched successfully', 'appstip-appointments' ),
