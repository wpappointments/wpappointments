...
pnpm -C plugins/appstip-appointments build        # Build plugin assets only
pnpm -C plugins/appstip-appointments css-types    # Generate CSS module type declarations
```
...
pnpm -C plugins/appstip-appointments test:e2e      # Playwright E2E tests
```

PHP tests run inside wp-env Docker container. Test files: `plugins/appstip-appointments/tests/php/` (suffix: `Test.php`). Framework: Pest (PHPUnit wrapper).
...
### Monorepo Structure
- `plugins/appstip-appointments/` — Main plugin (all core code lives here)
...
### PHP Backend (`plugins/appstip-appointments/src/`)
...
### TypeScript Frontend (`plugins/appstip-appointments/assets/`)
